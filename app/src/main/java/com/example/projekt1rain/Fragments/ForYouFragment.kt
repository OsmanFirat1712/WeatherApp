...
import com.example.projekt1rain.RetrofitApi.retrofitResponse3
...
class ForYouFragment() : Fragment(), CallBack, FragmentCallBack, RemoveCallBack {
    ...
    private val takeLat: MapViewFragment.Companion = MapViewFragment.Companion
    private val takeLon: Double = 0.0
    private val LatLng = MapViewFragment
    private val string = ""
...
companion object get{
    fun getonecall14(latitude: Double, longitude: Double, address: String) {
        val lat = retrofitResponse2().
        retrofitResponse2(latitude,longitude,address)
...
private fun getonecall() {
    ...
    favorites.forEach { favorite ->
        ...
        retrofitResponse2(lat!!,lon!!,adress)

        adapter3.notifyDataSetChanged()
        Toast.makeText(requireContext(), "$favorite oruspu permission granted", Toast.LENGTH_LONG).show()
...
/*
        ...
        private fun getone12() {
            val lat = favorites?.currentWeatherResponse?.lat!!
            val long = favorites?.currentWeatherResponse?.lon!!
            val address = favorites.address

            retrofitResponse2(lat,long,address)
...
*//*
            retrofitResponse2(takeLat!!, takeLat!!, string)
            Toast.makeText(requireContext(), "oruspu permission granted", Toast.LENGTH_LONG).show()
        ...
        override fun onComplete(favorites: List<Favorites>) {
            requireActivity().runOnUiThread(java.lang.Runnable {
                ...
                favorites.forEach { favorite ->
                    ...
                    val lon = favorite.currentWeatherResponse?.lon
                    val adress = favorite.address
                    retrofitResponse3(lat!!,lon!!,adress)
                    adapter3.updateforyou(favorites)
                    adapter3.notifyDataSetChanged()
...
/*
        ...
        override fun onFinish(favorites: Favorites) {
